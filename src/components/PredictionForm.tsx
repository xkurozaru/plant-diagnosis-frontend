...
import { Alert, AlertIcon, Box, Button, Heading, Image, Input, Tab, TabList, Tabs } from "@chakra-ui/react";
...
const PredictionForm = () => {
    ...
    useEffect(() => {
        const fetchModel = async () => {
            try {
                const res = await getPredictionModels(token)
    ...
    const handlePrediction = async (e: any) => {
        ...
        try {
            const res = await prediction(token, selectModelID, image)
    ...
    return (
        <Box maxW="fit-content" px="6" py="4">
            <Heading as="h2" size="lg">診断</Heading>
            ...
            <Box mt="5">
                <Image src={imagePreview} boxSize="224" borderRadius="10%" alt="image" />
            </Box>
            <Box mt="2">
                ...
                <Tabs isFitted variant="solid-rounded" colorScheme="cyan">
                    <TabList>
                        {predictionModels.map((model) => (
                            <Tab key={model.id} onClick={(e: any) => {
            ...
            <Box mt="5">
                <Button colorScheme='teal' variant='solid' size="lg" isDisabled={image === null || selectModelID === null} onClick={handlePrediction} isLoading={loading} loadingText="診断中">診断開始</Button>
            </Box>
        </Box >
    )
}
